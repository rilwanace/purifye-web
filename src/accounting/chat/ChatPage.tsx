    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100dvh - 60px)', background: '#131311' }}>
      <div style={{ padding: '12px 16px 8px', flexShrink: 0 }}>
        <div style={{
          ...
          <div onClick={() => setSubTab('entry')} style={segStyle(subTab === 'entry')}>Entry</div>
          <div onClick={() => setSubTab('history')} style={segStyle(subTab === 'history')}>History</div>
        </div>
      </div>

      {subTab === 'history' && (
        <div style={{ flex: 1, minHeight: 0, overflow: 'hidden' }}>
          <HistoryPage containerHeight="100%" />
        </div>
      )}
            ...
            <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(255,255,255,0.06)', background: '#1a1a18' }}>
              <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                {([
                  ...
                  { type: 'payment_received', label: 'Other', border: 'rgba(112,104,217,0.25)', bg: 'rgba(112,104,217,0.07)', color: '#7068D9' },
                ] as const).map((q: any) => (
