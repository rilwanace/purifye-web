...
export default function PersonalConfirmCard({ parsed, onClose, onSaved }: Props) {
  ...
  async function save() {
    ...
    try {
      ...
      if (workflow === 'tasks') {
        const f = { ...fields }
        if (f.target_value) (f as Record<string, unknown>).target_value = parseFloat(f.target_value) || null
        body.fields = f
      }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      ...
      <div style={{ background: '#1a1a18', borderRadius: '18px 18px 0 0', padding: '20px', maxHeight: '90dvh', overflowY: 'auto', animation: 'slideUp 0.3s ease' }}>
          ...
          {WORKFLOWS.map(wf => (
            <button
              ...
              style={{
                ...
                border: workflow === wf ? `1px solid ${ACCENT}33` : '1px solid transparent',
                background: workflow === wf ? `${ACCENT}1a` : 'rgba(255,255,255,0.04)',
        ...
        {/* Photo preview */}
        {parsed.photo_url && (
          <img src={parsed.photo_url} alt="uploaded" style={{ width: '100%', borderRadius: 10, marginBottom: 14, display: 'block', maxHeight: 160, objectFit: 'cover' }} />
        )}
