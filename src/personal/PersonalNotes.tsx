  ...
  if (selected) {
    return (
      <div style={{ padding: '16px 20px' }}>
        <button onClick={() => setSelected(null)} style={{ background: 'none', border: 'none', color: '#9c9b95', cursor: 'pointer', fontSize: 20, marginBottom: 12 }}>←</button>
        {selected.photo_url && (
        ...
        {selected.tags && selected.tags.length > 0 && (
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
            {selected.tags.map(tag => (
              <span key={tag} style={{ fontSize: 9, fontFamily: 'DM Mono', fontWeight: 700, background: `${ACCENT}1a`, color: ACCENT, borderRadius: 4, padding: '3px 8px' }}>
                {tag}
              </span>
            ))}
          </div>
    ...
    <div style={{ padding: '16px 20px' }}>
      ...
      {loading ? (
        ...
        <div style={{ fontSize: 12, fontFamily: 'DM Sans', color: '#6a6a64', textAlign: 'center', paddingTop: 40 }}>
          {search ? 'No results' : 'No notes yet — capture an idea below'}
        </div>
      ) : (
        notes.map(note => (
          <div
          ...
          >
            ...
            <div style={{ fontSize: 12, fontFamily: 'DM Sans', color: '#c4c3bc', lineHeight: 1.5,
            ...
            {note.tags && note.tags.length > 0 && (
              <div style={{ display: 'flex', gap: 4, marginTop: 6, flexWrap: 'wrap' }}>
                {note.tags.slice(0, 3).map(tag => (
                  <span key={tag} style={{ fontSize: 9, fontFamily: 'DM Mono', fontWeight: 700, background: `${ACCENT}1a`, color: ACCENT, borderRadius: 4, padding: '2px 6px' }}>
                    {tag}
                  </span>
                ))}
              </div>
